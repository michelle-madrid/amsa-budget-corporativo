# -*- coding: utf-8 -*-
"""
dividir_por_vp.py — Divide el dashboard en UN HTML por Vicepresidencia (solo su data).
=====================================================================================
Requiere haber corrido antes `python construir.py` (usa el dashboard ya generado).
Uso:  python dividir_por_vp.py

Lee  : salida/Dashboard Corporativo.html       (autocontenido, con detalle)
Escribe: salida/por_vp/<VP>/Dashboard <VP>.html    (uno por Vicepresidencia)
...
Reutiliza el embebido (gzip+base64) y las utilidades de construir.py.
"""
import os
import re
import json
...
import time
import shutil
import construir as B   # reutiliza SALIDA_HTML/SALIDA_DIR, DATA_UUID, ASSETS, _get_asset, _set_asset

OUT_DIR = os.path.join(B.SALIDA_DIR, "por_vp")
...
def main():
    ...
    for nombre in os.listdir(OUT_DIR):
        ...
        if (os.path.isfile(ruta) and nombre.lower().endswith(".html")) or os.path.isdir(ruta):
            _rm_resiliente(ruta)
    B.log(f"Dividiendo el dashboard en {len(por_vp)} Vicepresidencias…")
    generados = []
