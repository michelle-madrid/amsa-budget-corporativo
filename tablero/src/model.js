...
(function () {
  ...
  // Filtro «Otros Fletes»: whitelist de combinaciones (CECO, CLACO) de la VP Comercialización que
  // SÍ se muestran. El resto de la VP Comercialización se oculta cuando el filtro está activo.
  // La lista se edita en construir.py (SHOW_COMERCIAL) y llega por V.showCombos.
  const _showCombos = new Set((V.showCombos || []).map(a => a[0] + '\x01' + a[1]));
  const _comercialVp = V.comercialVp || 'VP Comercialización';
  // Grupo AMSA (nivel por encima de Vicepresidencia): dos categorías. Todas las VP van a
  // «AMSA Actividades Corporativas», salvo las de AMSA_OTROS que van a «Otros gastos AMSA».
  const AMSA_OTROS = new Set(['Exploraciones', 'Capex', '(sin VP)', 'Movimiento Financiero', 'Auditoría (HH Corporativo)', 'Michilla Costa']);
  const _grupoAmsa = vp => AMSA_OTROS.has(vp) ? 'Otros gastos AMSA' : 'AMSA Actividades Corporativas';
  ...
  let _hideFletes = true;      // Filtro «Otros Fletes»: en la VP Comercialización muestra SOLO _showCombos + Mano de Obra y oculta el resto. Por defecto ON.
  // Corte YTD 2026: hasta qué mes (1..12) suman el Real y el Ppto YTD. Editable en vivo desde el
  // dashboard. Por defecto = el corte del build (V.ytdHastaMes). Recalcula rec.y2026 desde rec.m26.
  let _ytdMes = +(V.ytdHastaMes || 5);
  const _real26MesMax = +(V.real26MesMax || 12);   // último mes con Real 2026 cargado (tope del selector)
  // Suma los meses 1.._ytdMes de rec.m26 (r=Real, p=Ppto) → {real:{n,a}, plan:{n,a}}. Si el registro
  // no trae m26 (reversas/dotaciones), cae a rec.y2026 (que ya viene con el corte del build).
  function _ytd26(r) {
    if (!r.m26) return r.y2026 || { real: { n: 0, a: 0 }, plan: { n: 0, a: 0 } };
    const side = s => {
      let n = 0, a = 0; const mm = r.m26[s] || {};
      for (const k in mm) { if (+k <= _ytdMes) { n += mm[k][0] || 0; a += mm[k][1] || 0; } }
      return { n, a };
    };
    return { real: side('r'), plan: side('p') };
  }
  // VP Asuntos Corporativos: mostrar SIEMPRE todos los Tipo Costo (C1/C2/C3), aunque el filtro
  // Tipo Costo esté acotado (p.ej. solo C1) para el resto de las VP. Por defecto ON.
  const AC_VP = 'VP Asuntos Corporativos';
  let _acTodoTC = true;
  ...
  function buildRecords() {
    ...
    return V.records.map((r, i) => {
      ...
      // Filtro «Otros Fletes» (global, ON por defecto): dentro de la VP Comercialización muestra SOLO
      // las combinaciones (CECO, CLACO) de la whitelist (construir.py → SHOW_COMERCIAL → V.showCombos)
      // MÁS los CLACOs de Mano de Obra; oculta el resto de la VP. Fuera de la VP Comercialización no afecta.
      if (_hideFletes && m.vp === _comercialVp
          && !_showCombos.has(r.ceco + '\x01' + r.claco)
          && (V.itemClas && V.itemClas[r.item]) !== 'Mano de Obra') hidden = true;
      ...
      const e26 = _ytd26(r);   // YTD 2026 (Real/Ppto) con el corte de mes activo (_ytdMes)
      ...
      return {
        ...
        vp: m.vp, _vp0: m.vp, gamsa: _grupoAmsa(m.vp), ger: m.ger, _ger0: m.ger, dceco: m.dceco || r.ceco, _dceco0: m.dceco || r.ceco, item: r.item,
        ...
        hist: { 2022: y(r.y2022), 2023: y(r.y2023), 2024: y(r.y2024), 2025: y(r.y2025), 2026: y(e26) },
        ...
        _realNA: { 2022: na(r.y2022 && r.y2022.real), 2023: na(r.y2023 && r.y2023.real), 2024: na(r.y2024 && r.y2024.real), 2025: na(r.y2025 && r.y2025.real), 2026: na(e26.real) },
        _planNA: { 2022: na(r.y2022 && r.y2022.plan), 2023: na(r.y2023 && r.y2023.plan), 2024: na(r.y2024 && r.y2024.plan), 2025: na(r.y2025 && r.y2025.plan), 2026: na(e26.plan) },
  ...
  function setHideFletes(b) { b = !!b; if (b !== _hideFletes) { _hideFletes = b; rebuild(); } }   // filtro «Otros Fletes»
  function setYtdMes(m) { m = +m; if (m >= 1 && m <= 12 && m !== _ytdMes) { _ytdMes = m; rebuild(); } }   // corte YTD 2026 en vivo
  // VP Asuntos Corporativos = todos los Tipo Costo. No toca los registros (es filtro en vivo),
  // así que NO hace falta rebuild(); el re-cálculo lo dispara el estado de React (deps).
  function setAcTodoTC(b) { _acTodoTC = !!b; }
  rebuild();  // init
  ...
  function _matchOpts(opts) {
    return rec => {
      if (opts.gamsas && opts.gamsas.length && !opts.gamsas.includes(rec.gamsa)) return false;   // filtro Grupo AMSA
      if (opts.vps && opts.vps.length && !opts.vps.includes(rec.vp)) return false;
      if (opts.exclVps && opts.exclVps.length && opts.exclVps.includes(rec.vp)) return false;   // excluir VPs (ej. off-shore en la Presentación Formato 2)
      ...
      // Tipo Costo (viene del CLACO). Excepción: si _acTodoTC está ON, VP Asuntos Corporativos
      // ignora este filtro y muestra todos sus Tipo Costo aunque el resto quede acotado a, p.ej., C1.
      if (opts.tcs && opts.tcs.length && !opts.tcs.includes(rec.tc)
          && !(_acTodoTC && rec.vp === AC_VP)) return false;
  ...
  function buildTree(opts) {
    ...
    const DISPM = { gamsa: n => n, vp: dispVP, ger: dispGer, dceco: n => n, itemrel: dispItemRel, item: dispItem, ceco: n => n, contra: n => n, claco: dispClaco, clacocod: n => n };
  ...
  function distribuible(opts) {
    ...
    const filtered = distRecords.filter(rec => {
      ...
      if (opts.items && opts.items.length && !opts.items.includes(rec.item)) return false;
      if (opts.tcs && opts.tcs.length && !opts.tcs.includes(rec.tc)
          && !(_acTodoTC && rec.vp === AC_VP)) return false;   // VP Asuntos Corporativos: todos los Tipo Costo
  ...
  function dimsFor(opts) {
    ...
    const gamsas = [], gamseen = new Set();   // Grupo AMSA (AMSA Actividades Corporativas / Otros gastos AMSA)
    recs.forEach(r => {
      if (r.gamsa && !gamseen.has(r.gamsa)) { gamseen.add(r.gamsa); gamsas.push(r.gamsa); }
    ...
    gamsas.sort((a, b) => a.localeCompare(b, 'es'));   // «AMSA Actividades…» < «Otros gastos…»
    return { gers, items, itemrels, tcs, clases, companias, cecos, clacos, aps, gamsas };
  }
  ...
  window.CORP = {
    ...
    setHideFletes, getHideFletes: () => _hideFletes,   // filtro «Otros Fletes»
    setYtdMes, getYtdMes: () => _ytdMes, getYtdMesMax: () => _real26MesMax,   // corte YTD 2026 en vivo
    setAcTodoTC, getAcTodoTC: () => _acTodoTC,   // VP Asuntos Corporativos = todos los Tipo Costo
