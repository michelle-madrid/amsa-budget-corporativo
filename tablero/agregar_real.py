...
Uso:  python agregar_real.py "uploads/real_total/Mi Real.xlsx" [--anio AAAA]
      --anio AAAA fuerza el año de atribución (a qué año del dashboard se suman las
      filas); si no se pasa, se detecta de la columna «Año» o del nombre de la hoja.
      El ajuste monetario siempre es a 2027 USD, independiente del año elegido.
"""
import os
import sys
import warnings
...
def calcular(df, anio, vec):
    ...
    # Listas de Python (int o None), NO Series: si la columna es float y tiene algún NaN,
    # .apply(_int) repromociona todo a float (6 → 6.0) y luego «{m:02d}» falla. Con listas, cada
    # elemento queda como int nativo.
    mes = [_int(x) for x in df[COL_MES]]
    yrs = [_int(x) for x in df[COL_ANIO]] if anio is None else [anio] * len(df)
    df["Año"] = yrs
...
def leer_y_calcular(path, vec, anio_forzado=None):
    """Lee todas las hojas de datos y devuelve un DataFrame único ya calculado.
    `anio_forzado` (int) obliga a atribuir TODAS las filas a ese año (ignora la
    columna «Año» y el nombre de la hoja). None → detecta como siempre."""
    xls = pd.ExcelFile(path)
    if "Consolidado" in xls.sheet_names:
        hojas = ["Consolidado"]
    else:
        hojas = [s for s in xls.sheet_names if str(s).strip().isdigit() and len(str(s).strip()) == 4]
        # Sin «Consolidado» ni hojas por año: si el libro tiene UNA sola hoja, usá esa.
        if not hojas and len(xls.sheet_names) == 1:
            hojas = [xls.sheet_names[0]]
    if not hojas:
        raise SystemExit("No encontré hoja «Consolidado», ni hojas por año (AAAA), y el archivo "
                         "tiene varias hojas. Dejá una sola hoja o nombrá la hoja con el año (AAAA).")
    frames = []
    for h in hojas:
        df = pd.read_excel(path, sheet_name=h)
        if anio_forzado is not None:
            anio = anio_forzado
        elif COL_ANIO in df.columns:
            anio = None
        elif str(h).strip().isdigit() and len(str(h).strip()) == 4:
            anio = int(str(h).strip())
        else:
            raise SystemExit("La hoja «%s» no trae columna «Año» ni se llama con un año (AAAA). "
                             "Elegí el año en el selector «Año» del panel (o agregá la columna «Año»)." % h)
        df = calcular(df, anio, vec)
        frames.append(df)
        origen = ("forzado %d" % anio_forzado) if anio_forzado is not None else ("columna" if anio is None else anio)
        print(f"    hoja «{h}»: {len(df)} filas (año {origen})")
...
def agregar_a_maestro(master_path, mapping, sub):
    ...
    # queda vacía al leerla con data_only=True desde construir.py.
...
def main():
    # Args: <archivo.xlsx> [--anio AAAA]. --anio fuerza el año de atribución de
    # las filas (a qué año del dashboard se suman); el ajuste monetario sigue a 2027 USD.
    args, anio_forzado = [], None
    i = 1
    while i < len(sys.argv):
        a = sys.argv[i]
        if a == "--anio":
            i += 1; anio_forzado = int(sys.argv[i])
        elif a.startswith("--anio="):
            anio_forzado = int(a.split("=", 1)[1])
        else:
            args.append(a)
        i += 1
    if not args:
        raise SystemExit("Uso: python agregar_real.py \"<archivo.xlsx>\" [--anio AAAA]")
    path = args[0]
    ...
    print("Procesando (ajuste a 2027):", os.path.basename(path),
          ("· año forzado %d" % anio_forzado) if anio_forzado is not None else "· año del archivo")
    df = leer_y_calcular(path, vec, anio_forzado)
