...
# ─────────────────────────── CONFIGURACIÓN ───────────────────────────
# Rutas de archivo (absolutas, relativas a la carpeta del motor `tablero/`, para que
# funcionen sea cual sea el cwd desde el que se invoque, ej. server.py / actualizar.py).
_UP = os.path.join(os.path.dirname(os.path.abspath(__file__)), "uploads")
BASE_XLSX   = os.path.join(_UP, "real_total", "Real_1000AC3510.xlsx")   # base de gastos (entrada = salida)
VECTOR_XLSX = os.path.join(_UP, "Vector Ajuste Data Histórica.xlsx")    # vector de factores
