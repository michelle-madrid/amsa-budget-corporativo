...
(function () {
  ...
  function buildRecords() {
    const cm = _cmap();
    const other = _cecoMode === 'old' ? (V.cecoNew || {}) : (V.cecoOld || {});  // fallback al otro mapa
    return V.records.map((r, i) => {
      const m = cm[r.ceco] || other[r.ceco] || { vp: '(sin VP)', ger: '(sin Gerencia)', tc: null, ap: null, cl: '(sin clasificación)', comp: (r.ceco || '').slice(0, 4) };
      ...
      return {
        id: i, src, comp, ceco: r.ceco, contra: r.contra || '(sin contrapartida)',
        vp: m.vp, _vp0: m.vp, ger: m.ger, item: r.item,
  ...
  function rebuild() {
    ...
    g.forEach(r => { records.push(r); (r.src === 'dist' ? distRecords : corpRecords).push(r); });
  ...
  function efectoMoneda(opts) {
    ...
    for (const rec of filtered) {
      ...
      if (wantFY && rec._fy26NA) { pptoN += rec._fy26NA.n; pptoA += rec._fy26NA.a; }
    }
  ...
  function buildTree(opts) {
    ...
    const DISPM = { vp: dispVP, ger: dispGer, item: dispItem, ceco: n => n, contra: n => n };
  ...
  // El "código" del Ítem Relevante ES la clave (Cód_Agrupación2). '(sin ítem)' → null.
